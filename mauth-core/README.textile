...
not the place that and authenticated user of the system lives.  A controller
...
HTTP status codes when non 200 is found.  To fail a login, or to force a login
at any point in your controller code, simply raise an Unauthenticated
exception, with an optional message and the user will be taken to the login
page.
...
that object does not evaluate to false.  For this reason, mauth-core does not
try to implement even a simple login form for you, since it may not be to your
liking.
...
<pre><code> Authentication.login_strategies.add(:password_login_from_form) do
...
controller instance whenever <code>session.authenticate(self)</code> is called
...
<pre><code> Authentication.login_strategies.add(:password_login_basic_auth) do
...
when we call <code>session.authenticate(self)</code>.  The first one that
returns a value that doesn't evaluate to false, will be considered the winna.
...
<code> Authentication.login_strategies.order </code> will return an array or
...
<code> Authenticateion.login_strategies.order = [:second, :first, :fourth]
</code>
...
data store, and also how to reconstruct it from that data.  To fail, just
return nil or raise an Unauthenticated exception.
...
class Authentication::Manager
  ...
  # return the value you want stored in the session def store_user(user)
...
found: <code> after :authenticate do |instance, *args|
  raise Merb::Controller::Unauthenticated, "User Not Active" unless
  instance.active?
end </code>
...
<code>session.authenticate(self)</code> inside a controller.  To logout use
<code>session.abandon!</code> and to force a login at any time use <code>raise
Unauthenticated, "You Aren't Cool Enough"</code>
